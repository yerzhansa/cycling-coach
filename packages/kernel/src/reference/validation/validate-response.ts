 ...
 */
export type ResponseHash = (response: string) => string;

export function parseMetaBlock(
  response: string,
  hashResponse: ResponseHash,
  ...
  if (blocks.length > 1) {
    console.warn(
      `Reference: reply carried ${blocks.length} ---meta--- blocks (expected 1); using the last. response=${hashResponse(response)}`,
    );
  }
