...
import { messageText } from "../src/sport/model-message.js";
import {
...
describe("computeHistoryTokenBudget", () => {
  it("computes window x ratio minus system-prompt tokens minus the reserve", () => {
    expect(computeHistoryTokenBudget({
...
describe("shouldCompact", () => {
  it("is strict at the boundary and counts the system prompt", () => {
    expect(shouldCompact({ messages: [msg(4000)], systemPrompt: "", contextWindowTokens: 21_200 })).toBe(false);
    expect(shouldCompact({ messages: [msg(4000)], systemPrompt: "", contextWindowTokens: 21_199 })).toBe(true);
    expect(shouldCompact({ messages: [msg(4000)], systemPrompt: "xxxx", contextWindowTokens: 21_200 })).toBe(true);
  });
});
