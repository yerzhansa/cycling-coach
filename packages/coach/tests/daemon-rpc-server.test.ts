...
describe.skipIf(!hasLoopback)("authenticated RPC projection", () => {
  ...
  it("dispatches strict named Planning v2 queries, commands, and results", async () => {
    const token = "x".repeat(43);
    ...
    const rejected = <Name extends PlanningV2Command["name"]>(request: {
      readonly name: Name;
    ...
    const planningV2: PlanningV2Operations = {
      "plan.list": list,
      "plan.get_context": getContext,
      "plan_creation.start": async (request) => rejected(request),
      "plan_creation.answer": async (request) => rejected(request),
      "plan_creation.preview": async (request) => rejected(request),
      "plan_creation.activate": async (request) => rejected(request),
      "plan_creation.discard": async (request) => rejected(request),
      "plan_change.preview": async (request) => rejected(request),
      "plan_change.apply": async (request) => rejected(request),
      "plan.close": async (request) => rejected(request),
    };
    const rpc = createCoachRpcServer({
      engine: engine(),
      operations: { ...operations, ...planningV2 },
      token,
      owner: "app-supervised",
    });
    const client = await openSocket(rpc);
    ...
    client.ws.send(
      JSON.stringify({
        ...
        id: "plan-list-v2",
        method: "plan.list",
        params: { schemaVersion: 2, closedCursor: null, closedLimit: 25 },
      }),
    );
    expect(parseCoachRpcEnvelope(await client.frames.next())).toMatchObject({
      id: "plan-list-v2",
      result: { schemaVersion: 2, activePlan: null, closedPlans: [] },
    });
    client.ws.send(
      JSON.stringify({
        jsonrpc: "2.0",
        id: "plan-context-v2",
        method: "plan.get_context",
        params: {
          schemaVersion: 2,
          target: {
            kind: "draft",
    ...
    expect(parseCoachRpcEnvelope(await client.frames.next())).toMatchObject({
      id: "plan-context-v2",
      result: { schemaVersion: 2, detail: { kind: "draft", revision: 1 } },
    });
    ...
    for (const [index, command] of commands.entries()) {
      ...
      expect(parseCoachRpcEnvelope(await client.frames.next())).toMatchObject({
        id: `plan-command-v2-${index}`,
        result: { schemaVersion: 2, name: command.name, status: "rejected" },
      });
    }
