...
const FIXTURE_PATH = resolve(HERE, "fixtures/golden/realistic-athlete.json");
const CHECKSUM_PATH = resolve(HERE, "fixtures/golden/realistic-athlete.json.sha256");

describe("realistic-athlete fixture checksum", () => {
  it("committed fixture bytes match the committed checksum", () => {
    ...
    const expected = readFileSync(CHECKSUM_PATH, "utf-8").trim().split(/\s+/)[0];
    expect(
      got,
      "Fixture bytes don't match the committed checksum.\n" +
        "Either the fixture was mutated in place (bad merge, editor save) — in which\n" +
        "case regenerate via `tools/sanitize-fixture.ts` — or the checksum is stale.\n" +
        "If you legitimately regenerated the fixture, run:\n" +
        "  (cd packages/core/tests/fixtures/golden && shasum -a 256 realistic-athlete.json > realistic-athlete.json.sha256)\n",
    ).toBe(expected);
  });
});
