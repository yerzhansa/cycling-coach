...
describe("first sync controller", () => {
  it("strictly validates completion and joins one synchronous coordinator flight", async () => {
    ...
    expect(ports.states).toEqual([{ status: "syncing" }]);
    coordinator.finish({ status: "succeeded", operation: 1, kind: "published", droppedActivities: { sourceRestricted: 0, other: 0, total: 0 } });
    await first;
  ...
  it("does not join or cancel a concurrently running manual sync for file-only setup", async () => {
    ...
    expect(ports.states).toEqual([]);
    expect(ports.focusComposer).toHaveBeenCalledTimes(1);
    coordinator.finish({ status: "succeeded", operation: 1, kind: "no-change", droppedActivities: { sourceRestricted: 0, other: 0, total: 0 } });
    await manualTask;
    expect(ports.states).toEqual([]);
  ...
  it.each([
    [
      { status: "succeeded", operation: 1, kind: "no-change", droppedActivities: { sourceRestricted: 0, other: 0, total: 0 } },
  ...
  it("retries only a retryable shared outcome and focuses the composer once", async () => {
    ...
    coordinator.finish({ status: "succeeded", operation: 2, kind: "published", droppedActivities: { sourceRestricted: 0, other: 0, total: 0 } });
    await retry;
    expect(coordinator.requestCalls).toEqual([1, 2]);
    expect(ports.focusComposer).toHaveBeenCalledTimes(1);
  });
  ...
  it.each(["owned", "joined"] as const)(
    ...
    async (admission) => {
      ...
      coordinator.finish({ status: "succeeded", operation: 2, kind: "no-change", droppedActivities: { sourceRestricted: 0, other: 0, total: 0 } });
      await laterManual;
  ...
  it("reactivates a completed first sync only for later onboarding without refocusing", async () => {
    ...
    coordinator.finish({ status: "succeeded", operation: 3, kind: "published", droppedActivities: { sourceRestricted: 0, other: 0, total: 0 } });
    await laterSetup;
