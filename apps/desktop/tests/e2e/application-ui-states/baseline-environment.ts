...
export function applicationBaselineForEnvironment(
  ...
  throw new Error(
    `No reviewed application baseline for ${environment.platform}/${environment.architecture} ${environment.darwinRelease}. Run on Darwin 25.2 or 25.6 arm64, or capture, inspect, and seal a separately approved environment baseline.`,
  );
}
