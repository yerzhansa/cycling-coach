...
4. **`desktop-windows-release.yml`** is the separate `workflow_dispatch` workflow for appending verified Windows assets to an existing published desktop release. The operator uploads the assets before it runs; the workflow verifies them and appends their verification record to the release body, and it never gates `desktop-release.yml`.
